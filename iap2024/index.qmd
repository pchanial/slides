...
<br/>
 ...
 - the product $P^{\textrm{T}} N^{-1} P$ is diagonal and can therefore be easily inverted (for pixel-pixel covariance matrix or preconditioning).
 - Each term is related to the number of times a pixel of the map has been observed.
...
#### Practical use case: Given two observations,
...
## Atmosphere Decontamination Using Time-Domain Component Separation for SO {style="font-size: 18px;"}

#### Science Goal
Characterize the observed atmospheric template from the recorded time-ordered data to separate the atmosphere from the sky signal we are after.
...
$$
\boxed{
\vec{d}_{\text{atm}} = \mathbf{A}(\text{PWV}) \mathbf{P}(w_x, w_y) s_{\text{atm}} + n
}
$$

- $\vec{d}_{\text{atm}}$: Atmospheric time-ordered data recorded by our telescope.
...
\end{bmatrix}
$$
- $s_{\text{atm}}$: Atmospheric template.
- $n$: Noise.
...
$$
\boxed{
\langle \delta \mathcal{S} \text{spec}(w_x, w_y, \text{PWV} \mid \vec{d}_{\text{atm}}) \rangle 
= \vec{d}_{\text{atm}}^\top (\mathbf{AP}) 
\Big( (\mathbf{AP})^\top \mathbf{N}^{-1} (\mathbf{AP}) \Big)^{-1} 
(\mathbf{AP})^\top \mathbf{N}^{-1} \vec{d}_{\text{atm}}
}
$$
...
Where:
  ...
  - $\mathbf{A}$: Mixing matrix.
  - $\mathbf{P}$: Pointing matrix.
  - $\mathbf{N}$: Noise covariance matrix.
$$
\mathbf{N} = 
\begin{bmatrix}
    \sigma_{1}^{2} & 0 \\
...
## Atmosphere Decontamination Using Time-Domain Component Separation for SO {style="font-size: 25px;"}
...
def average_spectral_likelihood_noise(d_atm, w_vec, PWV):
    ...
    A_93 = HomothetyOperator(atm_emission(PWV, mu_93)/atm_emission(PWV, mu_93), _in_structure=A_block_structure)
    A_145 = HomothetyOperator(atm_emission(PWV, mu_145)/atm_emission(PWV, mu_93), _in_structure=A_block_structure)
    ...
    N_93 = HomothetyOperator(noise_variance_93, _in_structure=N_block_structure)
    N_145 = HomothetyOperator(noise_variance_145, _in_structure=N_block_structure)
...
$\langle \delta \mathcal{S} \text{spec}(w_x, w_y \mid \text{PWV}_{\text{sim}}) \rangle$  
